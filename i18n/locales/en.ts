export default {
  title: 'Idea Portal',
  common: {
    typeMessage: 'Type a message...',
    signIn: 'Sign In',
    signUp: 'Sign Up',
    language: 'Language',
    selectLanguage: 'Select Language',
    // User menu translations
    user: 'User',
    profile: 'Profile',
    settings: 'Settings',
    signOut: 'Sign out',
    // User search translations
    searchUsers: 'Search users...',
    select: 'Select',
    selected: 'Selected',
    showBlocked: 'Show blocked users',
    friendsFirst: 'Friends first',
    noUsersFound: 'No users found',
    unknownUser: 'Unknown User',
    // Authentication providers
    google: 'Continue with Google',
    continueAsGuest: 'Continue as Guest',
    theme: {
      light: 'Light',
      dark: 'Dark'
    },
    // Registration translations
    register: 'Register',
    confirmPassword: 'Confirm Password',
    confirmPasswordInput: 'Confirm your password',
    passwordsDoNotMatch: 'Passwords do not match',
    registerError: 'Failed to create account',
    registerSuccess: 'Account created successfully',
    alreadyHaveAccount: 'Already have an account?',
    // Navigation translations
    lightMode: 'Light Mode',
    darkMode: 'Dark Mode',
    logout: 'Logout',
    noEmail: 'No email',
    newMessages: 'New Messages',
    unreadMessages: 'You have {count} unread messages',
    // Authentication translations
    email: 'Email',
    password: 'Password',
    emailInput: 'Enter your email',
    passwordInput: 'Enter your password',
    orContinueWith: 'Or continue with',
    dontHaveAccount: 'Don\'t have an account?',
    loginError: 'Invalid email or password',
    googleLoginError: 'Failed to sign in with Google',
    anonymousLoginError: 'Failed to sign in anonymously',
    loginSuccess: 'Successfully signed in',
    googleLoginSuccess: 'Successfully signed in with Google',
    anonymousLoginSuccess: 'Successfully signed in as guest',
    unexpectedError: 'An unexpected error occurred',
    // User card translations
    userAvatar: 'User avatar',
    anonymousUser: 'Anonymous User',
    openProfile: 'Open Profile',
    basicInfo: 'Basic Information',
    idInput: 'Enter ID',
    emailCannotBeChanged: 'Email cannot be changed',
    displayNameInput: 'Enter display name',
    verificationStatus: 'Verification Status',
    emailVerified: 'Email Verified',
    emailNotVerified: 'Email Not Verified',
    contacts: 'Contacts',
    phoneInput: 'Enter phone number',
    telegramInput: 'Enter Telegram username',
    whatsappInput: 'Enter WhatsApp number',
    viberInput: 'Enter Viber number',
    discordInput: 'Enter Discord username',
    linkedinInput: 'Enter LinkedIn profile URL',
    githubInput: 'Enter GitHub username',
    websiteInput: 'Enter website URL',
    connectedAccounts: 'Connected Accounts',
    saveChanges: 'Save Changes',
    error: 'Error',
    success: 'Success',
    invalidImageType: 'Invalid image type. Please upload an image file.',
    imageUploadSuccess: 'Image uploaded successfully',
    imageUploadError: 'Failed to upload image',
    profileUpdateError: 'Failed to update',
    id: 'ID',
    displayName: 'Display Name',
    phone: 'Phone',
    telegram: 'Telegram',
    whatsapp: 'WhatsApp',
    viber: 'Viber',
    discord: 'Discord',
    linkedin: 'LinkedIn',
    github: 'GitHub',
    website: 'Website',
    noEmailProvided: 'No email provided',
    accountStatus: 'Account Status',
    accountType: 'Account Type',
    disabled: 'Disabled',
    active: 'Active',
    anonymous: 'Anonymous',
    registered: 'Registered',
    notProvided: 'Not provided',
    memberSince: 'Member Since',
    noContactsProvided: 'No contacts provided',
    // Mail page translations
    searchByNameOrEmail: 'Search by name or email...',
    sortByMessageDate: 'By message date',
    sortByName: 'By name',
    sortByEmail: 'By email',
    sortByUnread: 'By unread',
    noChatsFound: 'No chats found',
    noActiveChats: 'No active chats',
    // Posts page translations
    myPosts: 'My Posts',
    searchByTitleOrId: 'Search by title or ID...',
    sortByCreationDate: 'By creation date',
    sortByViews: 'By views',
    sortByLikes: 'By likes',
    sortByComments: 'By comments',
    createProject: 'Create Project',
    createFirstProject: 'Create First Project',
    myProjects: 'My Projects',
    noProjects: 'You don\'t have any projects yet',
    projectsWhereIParticipate: 'Projects where I participate',
    notParticipating: 'You are not participating in any projects yet',
    edit: 'Edit',
    failedToLoadPosts: 'Failed to load posts',
    loading: 'Loading...',
    articles: 'Articles',
    createFirstArticle: 'Create First Article',
    noArticlesFound: 'No articles found for your query',
    failedToLoadArticles: 'Failed to load articles',
    chat: {
      title: 'Chat',
      scrollToBottom: 'Scroll to bottom',
      noEmail: 'No email',
      error: {
        loadingMessages: 'Error loading messages',
        markingAsRead: 'Error marking message as read',
        checkingUnread: 'Error checking unread messages'
      }
    },
    editProfile: 'Edit Profile',
    saving: 'Saving...',
    savingProfile: 'Saving profile...',
    profileUpdateSuccess: 'Profile updated successfully',
    create: 'Create',
    preview: 'Preview',
    aiAssistant: 'AI Assistant',
    restore: 'Restore',
    archive: 'Archive',
    publish: 'Publish',
    unpublish: 'Unpublish',
    update: 'Update',
    save: 'Save',
    idCopied: 'ID copied',
    idCopyError: 'Failed to copy ID',
    postUpdated: 'Post updated',
    postCreated: 'Post created',
    postPublished: 'Post published',
    postUnpublished: 'Post moved to drafts',
    postArchived: 'Post archived',
    postRestored: 'Post restored',
    saveError: 'Failed to save post',
    publishError: 'Failed to publish post',
    unpublishError: 'Failed to unpublish post',
    archiveError: 'Failed to archive post',
    restoreError: 'Failed to restore post',
    loadError: 'Failed to load post',
    search: 'Search',
    dateRange: 'Date Range',
    selectDate: 'Select date',
    advancedSearch: 'Advanced Search',
    reset: 'Reset',
    position: 'Position',
    positionInput: 'Enter your position',
    noPositionProvided: 'No position provided',
  },
  message: {
    status: {
      sending: 'Sending...',
      sent: 'Sent',
      delivered: 'Delivered',
      error: 'Error'
    },
    read: {
      single: 'read',
      multiple: 'reads'
    }
  },
  navigation: {
    title: 'Navigation',
    home: 'Home',
    ideas: 'Ideas',
    chat: 'Chat',
    profile: 'Profile',
    posts: 'My Posts',
    mails: 'Messages'
  },
  post: {
    author: 'Author',
    coAuthors: 'Co-authors',
    owner: 'Owner',
    authors: 'Authors',
    updated: 'Updated',
    views: 'Views',
    likes: 'Likes',
    create: {
      title: 'Title',
      titlePlaceholder: 'Enter article title',
      authors: 'Authors',
      addAuthor: 'Add author',
      cover: 'Cover',
      annotation: 'Annotation',
      annotationPlaceholder: 'Brief article description',
      keywords: 'Keywords',
      keywordsPlaceholder: 'Enter keywords separated by commas',
      add: 'Add',
      domain: 'Subject Area',
      domainPlaceholder: 'Enter subject area',
      content: 'Content',
      contentPlaceholder: 'Write article content in Markdown format',
      executionPolicy: 'Execution Policy',
      public: 'Public',
      contest: 'Contest',
      maxParticipants: 'Maximum number of participants',
      maxParticipantsPlaceholder: 'Leave empty for unlimited participants',
      maxParticipantsHelp: 'Leave the field empty for unlimited participants',
      deadline: 'Deadline',
      reset: 'Reset',
      addAuthors: 'Add Authors'
    },
    view: {
      noTitle: 'No title',
      cover: 'Cover',
      views: 'views',
      likes: 'likes',
      participants: 'participants'
    }
  },
  profile: {
    friends: 'Friends',
    noFriends: 'You don\'t have any friends yet',
    blockedUsers: 'Blocked Users',
    noBlockedUsers: 'You don\'t have any blocked users',
    addFriends: 'Add Friends',
    addToBlacklist: 'Add to Blacklist',
    removeFromFriends: 'Remove from Friends',
    removeFromBlacklist: 'Remove from Blacklist',
    removeFromFriendsConfirm: 'Are you sure you want to remove this user from friends?',
    removeFromBlacklistConfirm: 'Are you sure you want to remove this user from blacklist?',
    cancel: 'Cancel',
    remove: 'Remove',
    success: 'Success',
    userRemovedFromFriends: 'User removed from friends',
    userRemovedFromBlacklist: 'User removed from blacklist',
    usersAddedToFriends: 'Users added to friends',
    usersAddedToBlacklist: 'Users added to blacklist',
    friendsFetchError: 'Failed to load friends',
    blockedUsersFetchError: 'Failed to load blocked users',
    addToFriendsError: 'Failed to add users to friends',
    addToBlacklistError: 'Failed to add users to blacklist',
    removeFromFriendsError: 'Failed to remove user from friends',
    removeFromBlacklistError: 'Failed to remove user from blacklist'
  },
  index: {
    title: 'Home',
    title1: 'Share your ideas',
    title2: 'and find like-minded people',
    subtitle: 'Join the community of innovators and entrepreneurs',
    button1: 'Create your idea with',
    button2: 'AI assistant',
    searchPlaceholder: 'Search for projects...',
    stepper: {
      register: {
        title: 'Register',
        description: 'Create an account in minutes'
      },
      profile: {
        title: 'Complete your profile',
        description: 'Tell us about yourself and your interests'
      },
      explore: {
        title: 'Explore ideas',
        description: 'Discover projects from other members'
      },
      create: {
        title: 'Create your idea',
        description: 'Share your project with the community'
      }
    }
  },
  core: {
    desc: 'Our AI assistant will help you create and structure your project, suggest improvements, and connect you with potential team members.'
  },
  aiAgent: {
    resetChat: 'Reset chat and generate new token',
    clearHistory: 'Clear chat history',
    chatReset: 'Chat has been reset successfully',
    resetError: 'Failed to reset chat',
    welcomeMessage: 'Hello! I am your AI assistant. I am ready to help you with your questions and tasks. How can I help you today?'
  },
  feedback: {
    title: 'Feedback',
    description: 'Share your thoughts, suggestions, or report an issue',
    template: 'Template for filling',
    templateDescription: 'Download the template for filling out feedback',
    downloadTemplate: 'Download template',
    fillForm: 'Fill Google Form',
    writeDeveloper: 'Found a bug or have a suggestion? Contact the developer',
    leaveFeedback: 'Leave Feedback'
  },
  subjectAreas: {
    languages: 'Languages',
    'languages.english': 'English',
    'languages.russian': 'Russian',
    'languages.chinese': 'Chinese',
    'languages.spanish': 'Spanish',
    'languages.french': 'French',
    'languages.german': 'German',
    'languages.japanese': 'Japanese',
    'languages.arabic': 'Arabic',
    'languages.italian': 'Italian',
    'languages.portuguese': 'Portuguese',
    programming: 'Programming',
    'programming.web': 'Web Development',
    'programming.mobile': 'Mobile Development',
    'programming.backend': 'Backend Development',
    'programming.frontend': 'Frontend Development',
    'programming.data_science': 'Data Science',
    'programming.ai': 'Artificial Intelligence',
    'programming.ml': 'Machine Learning',
    'programming.game': 'Game Development',
    'programming.devops': 'DevOps',
    'programming.security': 'Security',
    'programming.algorithms': 'Algorithms & Data Structures',
    'programming.testing': 'Testing & QA',
    networking: 'Network Administration',
    'networking.network_security': 'Network Security',
    'networking.cloud': 'Cloud Computing',
    'networking.system_admin': 'System Administration',
    'networking.virtualization': 'Virtualization',
    'networking.wireless': 'Wireless Networks',
    'networking.routing': 'Routing & Switching',
    'networking.voip': 'VoIP',
    'networking.linux': 'Linux Administration',
    'networking.windows': 'Windows Administration',
    engineering: 'Engineering',
    'engineering.mechanical': 'Mechanical Engineering',
    'engineering.electrical': 'Electrical Engineering',
    'engineering.civil': 'Civil Engineering',
    'engineering.chemical': 'Chemical Engineering',
    'engineering.software': 'Software Engineering',
    'engineering.aerospace': 'Aerospace Engineering',
    'engineering.biomedical': 'Biomedical Engineering',
    'engineering.environmental': 'Environmental Engineering',
    'engineering.industrial': 'Industrial Engineering',
    'engineering.robotics': 'Robotics',
    biology: 'Biology',
    'biology.genetics': 'Genetics',
    'biology.microbiology': 'Microbiology',
    'biology.biochemistry': 'Biochemistry',
    'biology.molecular': 'Molecular Biology',
    'biology.zoology': 'Zoology',
    'biology.botany': 'Botany',
    'biology.ecology': 'Ecology',
    'biology.physiology': 'Physiology',
    'biology.cell': 'Cell Biology',
    'biology.evolution': 'Evolutionary Biology',
    medicine: 'Medicine',
    'medicine.general': 'General Medicine',
    'medicine.surgery': 'Surgery',
    'medicine.pediatrics': 'Pediatrics',
    'medicine.psychiatry': 'Psychiatry',
    'medicine.neurology': 'Neurology',
    'medicine.cardiology': 'Cardiology',
    'medicine.oncology': 'Oncology',
    'medicine.dentistry': 'Dentistry',
    'medicine.pharmacy': 'Pharmacy',
    'medicine.public_health': 'Public Health',
    'medicine.medical_research': 'Medical Research',
    sports: 'Sports & Physical Education',
    'sports.sports_science': 'Sports Science',
    'sports.coaching': 'Coaching',
    'sports.physical_therapy': 'Physical Therapy',
    'sports.sports_medicine': 'Sports Medicine',
    'sports.fitness': 'Fitness Training',
    'sports.nutrition': 'Nutrition',
    'sports.kinesiology': 'Kinesiology',
    'sports.athletic_training': 'Athletic Training',
    'sports.sports_management': 'Sports Management',
    geology: 'Geology & Earth Sciences',
    'geology.geology': 'Geology',
    'geology.geophysics': 'Geophysics',
    'geology.geochemistry': 'Geochemistry',
    'geology.paleontology': 'Paleontology',
    'geology.seismology': 'Seismology',
    'geology.oceanography': 'Oceanography',
    'geology.meteorology': 'Meteorology',
    'geology.hydrology': 'Hydrology',
    'geology.environmental_geology': 'Environmental Geology',
    architecture: 'Architecture & Construction',
    'architecture.architecture': 'Architecture',
    'architecture.urban_planning': 'Urban Planning',
    'architecture.landscape': 'Landscape Architecture',
    'architecture.interior': 'Interior Design',
    'architecture.civil_construction': 'Civil Construction',
    'architecture.structural': 'Structural Engineering',
    'architecture.building_tech': 'Building Technology',
    'architecture.sustainable': 'Sustainable Design',
    design: 'Design & Arts',
    'design.graphic': 'Graphic Design',
    'design.industrial': 'Industrial Design',
    'design.fashion': 'Fashion Design',
    'design.interior': 'Interior Design',
    'design.animation': 'Animation',
    'design.photography': 'Photography',
    'design.fine_arts': 'Fine Arts',
    'design.digital_arts': 'Digital Arts',
    'design.uiux': 'UI/UX Design',
    economics: 'Economics & Management',
    'economics.economics': 'Economics',
    'economics.finance': 'Finance',
    'economics.accounting': 'Accounting',
    'economics.marketing': 'Marketing',
    'economics.management': 'Management',
    'economics.entrepreneurship': 'Entrepreneurship',
    'economics.business_analytics': 'Business Analytics',
    'economics.hr': 'Human Resources',
    education: 'Education & Pedagogy',
    'education.ed_tech': 'Educational Technology',
    'education.curriculum': 'Curriculum Development',
    'education.special_ed': 'Special Education',
    'education.early_childhood': 'Early Childhood Education',
    'education.higher_ed': 'Higher Education',
    'education.ed_psychology': 'Educational Psychology',
    'education.distance': 'Distance Learning',
    physics: 'Physics & Mathematics',
    'physics.theoretical': 'Theoretical Physics',
    'physics.applied': 'Applied Physics',
    'physics.astrophysics': 'Astrophysics',
    'physics.quantum': 'Quantum Mechanics',
    'physics.pure_math': 'Pure Mathematics',
    'physics.applied_math': 'Applied Mathematics',
    'physics.statistics': 'Statistics',
    chemistry: 'Chemistry',
    'chemistry.organic': 'Organic Chemistry',
    'chemistry.inorganic': 'Inorganic Chemistry',
    'chemistry.analytical': 'Analytical Chemistry',
    'chemistry.physical': 'Physical Chemistry',
    'chemistry.biochemistry': 'Biochemistry',
    'chemistry.chemical_engineering': 'Chemical Engineering',
    social_sciences: 'Social Sciences',
    'social_sciences.sociology': 'Sociology',
    'social_sciences.psychology': 'Psychology',
    'social_sciences.political': 'Political Science',
    'social_sciences.anthropology': 'Anthropology',
    'social_sciences.history': 'History',
    'social_sciences.law': 'Law',
    'social_sciences.linguistics': 'Linguistics'
  }
} 